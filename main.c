...
void TaskD() //creating the TaskC function which is void.
{
    static uint8_t taskcmsg[] = "Task D\n"; //The message to be displayed in renode.
    sendUART(taskcmsg, sizeof(taskcmsg));
    //ReRunMe(0);
}
...
void Dispatch() //the dispatch function.
{
    ...
    else if (Queue[dispatched].task == &TaskD)
    {
        TaskD();
    }
}
...
void init()
{
    QueTask(TaskA, 6); //setting quetask 
    QueTask(TaskB, 5);
    QueTask(TaskC, 1);
    QueTask(TaskD, 3);
}
...
//	  Dispatch(); //we can call this function to run one instance of this code.
    ...
    while (1)
    {
        ...
        dispatched++; //increasing the dispatched counter
    }
